...
def get_sentinel2_image(
    ...
    """
    Retrieve a cloud-filtered Sentinel-2 SR median composite for the given AOI.
    ...
    Collection:
        ...
        harmonised across processing baselines. Key bands:
          B2=Blue, B3=Green, B4=Red, B5=RedEdge1, B8=NIR, B11=SWIR1, B12=SWIR2
        Values are scaled integers; divide by 10000 for reflectance [0, 1].
    """
    ...
    try:
        ...
        if count == 0:
            logger.warning(
                "Sentinel-2: no images found for (%s, %s) between %s and %s",
        ...
        logger.info(
            "Sentinel-2 composite created from %d images (lat=%.4f, lon=%.4f)",
            count, lat, lon,
        )
...
def get_all_tile_layers(
    ...
    Example usage in app.py:
        tiles = get_all_tile_layers(lat, lon,
                    {"start": "2019-01-01", "end": "2019-12-31"},
                    {"start": "2024-01-01", "end": "2024-12-31"})
        for layer_name, tile in tiles.items():
            if tile and show_flags.get(layer_name, True):
                add_ee_layer_to_folium(folium_map, tile, opacity=0.7)
    """
    if not _is_gee_ready():
    ...
    logger.info("Fetching Sentinel-2 before image (%s → %s) …",
                before_dates.get("start"), before_dates.get("end"))
    img_before, geom_before = get_sentinel2_image(
        lat, lon,
    ...
    logger.info("Fetching Sentinel-2 after image (%s → %s) …",
                after_dates.get("start"), after_dates.get("end"))
    img_after, geom_after = get_sentinel2_image(
        lat, lon,
    ...
    logger.info("Fetching SRTM DEM …")
    dem, geom_dem = get_elevation(lat, lon, buffer_km)

    # ── Layer 1: NDVI (Before) ────────────────────────────────────────────────
    try:
        if img_before is not None and geom_before is not None:
    ...
    # ── Layer 2: NDVI (After) ─────────────────────────────────────────────────
    try:
        if img_after is not None and geom_after is not None:
